...
def extract_features(window: pd.DataFrame):
    class_a = list(adx_and_market_strength(window))            # ADX
    class_b = list(entropy_and_chaos(window))                  # Entropy
    class_c = list(gap_range_and_price_levels(window))         # Gaps and range
    class_d = list(macd_based_momentum_summary(window))        # MACD momentum
    class_e = list(price_action_and_trend_features(window))    # Trend + price action
    class_f = list(volatility_and_noise(window))               # Noise/Volatility
    class_g = list(wick_behavior_and_volatility(window))       # Wick behavior
